...
class DES : public CipherInterface {
    private:
        ...
        string intialize(string pt){
            string plaintext;
            if (pt.length()==16) {
                for (int i=0; i<16; i++) plaintext+=hex_to_bin(pt[i]);
            }
                ...
                pt = ss.str();
                for (int i=0; i<16; i++) plaintext+= (pt.length()>i) ? hex_to_bin(pt[i]) : hex_to_bin('0');
            } else throw invalid_argument("Plaintext size is invalid (must be less than 16 for string and 16 characters for HEX)");
            return plaintext;
        }
        ...
        void key_generation(string ky){
            keys.clear();
            if (ky.length()!=16) throw invalid_argument("Key size must be 16 characters in hexadecimal format.");
            string key;
            for (int i=0; i<16; i++) key+=hex_to_bin(ky[i]);
        ...
        string crypt(string pt, string ky, bool is_encrypting){
