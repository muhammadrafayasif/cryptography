...
class AES : public CipherInterface {
    private:
        ...
        string get_answer(bool is_encrypting=false){
            string cipher;
            for (int i=0; i<4; i++){
                for (int j=0; j<4; j++){
        ...
        void setup(string text, string key){
            if (text.length()==32){
                int k=0;
            ...
            if (key.length()==32){
                ...
                for (int i=16; i<32; i++) key[i]=0;
            }

            int k=0;
            for (int i=0; i<4; i++){
                for (int j=0; j<4; j++){
                    state[j][i] = (text.length()>k) ? (uint8_t)text[k] : 20;
                    keys[0][j][i] = (key.length()>k) ? (uint8_t)key[k] : 20;
                    k++;
                }
            }
        }
